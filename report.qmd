...
tourism_filtered <- left_join(tourism, aus_map, by = c("Region"))
...
wrangled_data <- left_join(quarter_merged, tourism_filtered, by = c("Quarter", "State")) %>%
...
region_subset <- wrangled_data %>%
  filter(State %in% c("New South Wales", "Queensland")) %>%
...
region_temporal <- region_subset %>%
  ...
  group_by(Year, Quarter, State, Purpose) %>%
...
ggplot(region_subset, aes(x = State)) +
  geom_bar(stat = "count") +
...
plot2 <- ggplot(region_temporal, aes(
  x = Quarter,
  y = Trips,
  group = State
)) +
...
selected_state <- "Queensland"

filtered_data <- wrangled_data %>%
  filter(State == selected_state)

selected_regions <- c("Brisbane City", "Brisbane Airport", "Logan Village")
...
# Create a bar plot to show the count of each purpose within the selected region
ggplot(filtered_data, aes(x = eventDate, y = Trips, color = Purpose)) +
  geom_line() +
  labs(x = "Event Year", y = "Number of Trips") +
  ggtitle("Number of Trips by Purpose") +
...
region_subset <- region_subset %>%
  mutate(Year = year(eventDate)) %>%
  group_by(Year) %>% 
  mutate(wild_sighting= sum(occurrenceStatus),
        tourist_trips = sum(Trips))

# Create a line plot with log-scaled y-axis
ggplot(region_subset, aes(x = Year)) +
  geom_line(aes(y = tourist_trips, color = "Tourist Trips"), linetype = "solid") +
  geom_line(aes(y = wild_sighting, color = "Wild Sightings"), linetype = "solid") +
  labs(x = "Year", y = "Count") +
  ggtitle("Ecotourist Interest in Wild Thing by Year") +
  scale_color_manual(values = c("Tourist Trips" = "blue", "Wild Sightings" = "red")) +
  theme_minimal() +
  scale_x_continuous(breaks = unique(region_subset$Year))
```
# Regression Model 
...
The above is a "custom Function" created to apply in rest of all the plot to ease on the task and make plots more appealing. 


```{r}
#wrangling the tourism datasets for the further analysis
australia_map <- read_sf(here::here("data/SA2_2021_AUST_SHP_GDA2020/SA2_2021_AUST_GDA2020.shp")) %>%
  select("SA2_NAME21", "STE_NAME21") %>%
  rename(
    Region = SA2_NAME21,
    State = STE_NAME21
  )
...
#| label: fig-tourism
#| fig-cap: "The trend in Tourism Over Quarter"

p1 <- tourist_filtered %>%
  group_by(Year) %>%
  summarize(TotalTrips = sum(Trips), Size = max(TotalTrips)) %>%
  ggplot(aes(x = Quarter, y = TotalTrips, size = Size)) +
  geom_point(color = "blue") +
  labs(
    title = "Tourism Patterns Across Recent Quarters",
    x = "Quarters",
    y = "Total Trips"
  ) + custom_theme() 
...
@fig-tourism shows the trend of tourism over quarters. 


:::{.callout-note}
# Key takeaway

The plot above illustrates the quarterly patterns of tourism over multiple years. A noteworthy observation from the plot is that, throughout the displayed years, the number of trips consistently exceeded a threshold of 50,00,000. Remarkably, this trend remained resilient even during the challenging times of the COVID-19 pandemic, as there is no significant decline in the number of trips.
...
# Creating a Scatter Plot for Data Visualization
#| label: fig-sighting
#| fig-cap: "The trend in Sightings across Quarters"
p2 <- merged_quarter %>%
  group_by(Year, Quarter) %>%
  summarize(Total_occurrenceStatus = sum(occurrenceStatus), Size = max(Total_occurrenceStatus)) %>%
  ggplot(aes(x = Quarter, y = Total_occurrenceStatus, size = Size)) +
  geom_point(color = "red") +
  labs(
    title = "Sightings Patterns Across Recent Quarters",
    x = "Quarters",  # Change x-axis label to "Quarters"
    y = "Sightings"
  ) + custom_theme()
...
p2_plotly
```
@fig-sighting demonstrates the trend of sightings across quarters 


:::{.callout-note}
# Key takeaway

This scatter plot depicts the relationship between wildlife sightings and quarters. Unfortunately, the data does not reveal any prominent patterns. The primary observation is that for the majority of the years, there were no sightings, except during the years of the COVID-19 pandemic, which occurred from 2020 to 2022.
