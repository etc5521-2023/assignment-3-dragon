...
tourism_filtered <- left_join(tourism, aus_map, by = c("Region" = "SA2_NAME21")) %>%
  rename(State = Region)
```
...
#mutate the data 
merged_final <- final_Euoplos_Rainbow %>%
  mutate(
    Quarter = as.yearqtr(eventDate)
  ) 
merged_final$Quarter <- yearquarter(merged_final$Quarter)

#joining the data to sightings dataset
merged_data <- left_join(merged_final, tourism_filtered, by = c("Quarter", "State"))
